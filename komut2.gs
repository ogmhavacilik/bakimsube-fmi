...
function handleGetInitData() {
  ...
  try {
    ...
    try {
      cleanErroneousYoklamaRows();
    } catch(errClean) {
      console.error("Yoklama temizleme hatası: " + errClean.message);
    }
...
function cleanErroneousYoklamaRows() {
  try {
    const mainDb = getMainDb();
    const sheet = getSheetSafely(mainDb, ["YOKLAMA AKTAR", "Yoklama", "YOKLAMA", "ATTENDANCE"]);
    ...
    const headers = data[0].map(h => String(h).trim().toUpperCase());
    const dutyTypCol = headers.indexOf("DUTY_TYP") !== -1 ? headers.indexOf("DUTY_TYP") : headers.indexOf("DUTY_TYPE");
    const statusCol = headers.indexOf("STATUS");
    const noteCol = headers.indexOf("NOTE");
    
    for (let i = data.length - 1; i >= 1; i--) {
      const statusVal = statusCol !== -1 ? String(data[i][statusCol] || "").toUpperCase() : "";
      const dutyTypVal = dutyTypCol !== -1 ? String(data[i][dutyTypCol] || "").toUpperCase() : "";
      const noteVal = noteCol !== -1 ? String(data[i][noteCol] || "").toUpperCase() : "";
      
      // Delete any erroneous row where STATUS is "Görev" and DUTY_TYPE is "Plansız" (generated by FMI/Earned overtime saves)
      // OR NOTE contains "mesai" / "kazanım"
      if ((statusVal.includes("GÖREV") || statusVal.includes("GOREV")) && dutyTypVal.includes("PLANSIZ")) {
        sheet.deleteRow(i + 1);
      } else if (noteVal.includes("MESAI") || noteVal.includes("MESAİ") || noteVal.includes("KAZANIM") || noteVal.includes("KAZANİM")) {
        sheet.deleteRow(i + 1);
      }
    }
  } catch(e) {
    console.warn("Failed to clean erroneous yoklama rows: " + e.message);
  }
}
...
function writeToAttendance(tx) {
  ...
  // KESİNLİKLE VE SADECE İZİN KAYDI MODÜLÜNDEN GİRİLEN İZİN KULLANIMI (USED) YOKLAMAYA YAZILABILIR!
  // Mesai Kazanımı, FMİ Girişi, Plansız Mesai, Earned vb. izin kaydı dışındaki HİÇBİR veri Yoklama sayfasına YAZILAMAZ!
  const isUsed = typeValue === "USED" || typeValue.includes("IZIN") || typeValue.includes("İZİN") || desc.includes("KULLANIM") || desc.includes("KULLANILAN");
  const isEarned = typeValue === "EARNED" || typeValue.includes("KAZANIM") || typeValue.includes("ODEME") || typeValue.includes("ÖDEME") || desc.includes("KAZANIM") || desc.includes("GIRIS") || desc.includes("GİRİŞ");

  if (!isUsed || isEarned) {
    console.log("İzin Kullanımı (USED) olmayan veya Mesai Kazanımı olan kayıt Yoklama sayfasına yazılmayacak: " + pid + " " + dateVal);
    return;
  }
